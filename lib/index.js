...
import { regexInt, regexDouble, regexTime, regexDate, regexDateTime, localToUtc, bit_clear_and_set, $bit_clear_and_set, $bit_unset } from './util';
...
export function verifyFormFields(message, descriptor, root) {
    ...
    if (message_.vfbs) {
        if (root_.msg) {
            $bit_unset(root_, 'state', 7 /* MASK_STATUS */);
            root_.msg = '';
        }
        return false;
    }
    ...
    if (!(rfbs = descriptor.$rfbs) || rfbs === message_.rfbs)
        return true;
    if (!root_.msg) {
        $bit_clear_and_set(root_, 'state', 7 /* MASK_STATUS */, 2 /* ERROR */);
...
export function formSuccess(pojo, update) {
    var pojo_ = pojo['_'];
...
// =====================================
// event handling
function postValidate(message, fd, f, fk, msg, root) {
    var message_ = message._, root_ = root._, state = message_.state, vfbs = message_.vfbs, rfbs = message_.rfbs, required = fd.m === 2, flag = 1 << (f - 1);
    message_[fk] = msg;
    if (!(state & 16 /* UPDATE */))
