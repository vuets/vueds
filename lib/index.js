...
export function formUpdate(pojo, pager, original, changes) {
    ...
    if ((state & 8 /* LOADING */) || !verifyFormFields(pojo, $d, true))
        return undefined;
    var mc = MultiCAS.$create(), diffCount = diffVmTo(mc, $d, original, pojo);
...
// =====================================
// event handling
function postValidate(message, fd, f, fk, message_, msg, root, set) {
    var root_ = root._, state = message_.state, sfbs = message_.sfbs, vfbs = message_.vfbs, rfbs = message_.rfbs, required = fd.m === 2, flag = 1 << (f - 1);
    message_[fk] = msg;
    if (!(state & 16 /* UPDATE */))
...
function validateString(val, message, fd, f, fk, message_, prop, el, update, root) {
...
function validateFloat(val, message, fd, f, fk, message_, prop, el, update, root) {
    var msg = null, set = !!val, v;
    if (set) {
...
function validateInt(val, message, fd, f, fk, message_, prop, el, update, root) {
    var msg = null, set = !!val, v;
    if (set) {
...
function validateTime(val, message, fd, f, fk, message_, prop, el, update, root) {
    var msg = null, set = !!val, v;
    if (set) {
...
function validateDate(val, message, fd, f, fk, message_, prop, el, update, root) {
    var msg = null, set = !!val, v;
    if (set) {
...
function validateDateTime(val, message, fd, f, fk, message_, prop, el, update, root) {
    var msg = null, set = !!val, v;
    if (set) {
...
export function $change(e, message, field, update, root) {
    ...
    if (!fd || fd.t === 2 /* BYTES */)
        return null;
    var message_ = message['_'], f = fd._, prop = fd.$ || fk, el = e.target, msg = null, val;
    switch (fd.t) {
        case 1 /* BOOL */:
