...
// =====================================
// event handling
function postValidate(message, fd, fk, f, flag, message_, sfbs, msg, root, set) {
...
function validateString(val, message, fd, fk, f, flag, message_, prop, el, update, root) {
    var msg = null, set = !!val, sfbs = message_.sfbs, v;
    if (set) {
        // backup data
...
function validateFloat(val, message, fd, fk, f, flag, message_, prop, el, update, root) {
    var msg = null, set = !!val, sfbs = message_.sfbs, v;
    if (set) {
        // backup data
        if (update && (v = message[prop]) !== undefined && v !== message_[prop])
...
function validateInt(val, message, fd, fk, f, flag, message_, prop, el, update, root) {
    var msg = null, set = !!val, sfbs = message_.sfbs, v;
    if (set) {
        // backup data
        if (update && (v = message[prop]) !== undefined && v !== message_[prop])
...
function validateTime(val, message, fd, fk, f, flag, message_, prop, el, update, root) {
    var msg = null, set = !!val, sfbs = message_.sfbs, v;
    if (set) {
        // backup data
        if (update && (v = message[prop]) !== undefined && v !== message_[prop])
...
function validateDate(val, message, fd, fk, f, flag, message_, prop, el, update, root) {
    var msg = null, set = !!val, sfbs = message_.sfbs, v;
    if (set) {
        // backup data
        if (update && (v = message[prop]) !== undefined && v !== message_[prop])
...
function validateDateTime(val, message, fd, fk, f, flag, message_, prop, el, update, root) {
    var msg = null, set = !!val, sfbs = message_.sfbs, v;
    if (set) {
        // backup data
        if (update && (v = message[prop]) !== undefined && v !== message_[prop])
...
export function $change(e, message, field, update, root) {
    ...
    if (!fd || fd.t === 2 /* BYTES */)
        return null;
    var message_ = message['_'], f = fd._, flag = 1 << (f - 1), prop = fd.$ || fk, el = e.target, msg = null, val;
    switch (fd.t) {
        case 1 /* BOOL */:
